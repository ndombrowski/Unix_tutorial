...
One example unix tutorial can be found [here](http://www.ee.surrey.ac.uk/Teaching/Unix/index.html)
...
The ``wc`` (=wordcount) command in UNIX is a command line utility for printing newline, word and byte counts for files. It can return the number of lines in a file, the number of characters in a file and the number of words in a file. It can also be combine with pipes for general counting operations.
...
A useful thing of echo is that we can use it to add rows, i.e. a header to our data
...
Here, you see what this command does steo by step:
...
Notice, that here we need to add the file extension!


While loops (usable example to follow):
