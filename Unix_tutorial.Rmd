...
- ``pwd`` = print working directory, find out where we are. Usually when we login we start from our home directory ()
  - This is /export/lv1/user/your_name on ada 
  - This will be something like /Users/ninadombrowski on your own computer
...
#on ada, move the spang directory and into the project directory
...
For example on ada the full path to our home directory is:
``/export/lv1/user/ndombrowski``

If we want to go to the desktop using an absolute path we do

```bash
cd /export/lv1/user/ndombrowski/Desktop
```
...
############################################
############################################
# Working on the NIOZ servers
############################################
############################################

We have two computational systems we can use at NIOZ. Some basic info about them can be found [here](http://laplace.nioz.nl/HPC/index.html) (can only be viewed via VPN or NIOZ network):

- If you need access to our servers, the first person to contact is Hans Malschaert (hans.malschaert@nioz.nl). He will set up an account that will work for all systems we have at NIOZ.

- To access the servers from home you need to set up a VPN. Information on how to do this can be found here:
  
    - Access the [intranet](https://intranet.nioz.nl)
    - Follow the instructions [here](https://intranet.nioz.nl/vpn)

1. ada

- We can work directly from the command line
- 1 node, 144 CPUS
- ~800 GB memory
- Usage: Try not use more than 30-40% of resources


2. laplace

- We can not work from the log in node but need to submit via a slurm submission system
- 12 nodes (11 working nodes = no61-72) --> each with each 96GB memory and 80 "cpus""
- ~800 GB memory
- Usage: Use submission system, do not use more than 3-4 nodes
- Different nodes allow to run jobs for different lengths:

<p align="left">
  <img width="400" height="400" src="/Users/ninadombrowski/Desktop/WorkingDir/Notebooks/Pictures/lapalce.png">
</p>
...
```bash
#connect to ada
ssh -X username@ada.nioz.nl

#connect to laplace.
ssh -X username@laplace.nioz.nl
...
<p align="left">
  <img width="400" height="400" src="/Users/ninadombrowski/Desktop/WorkingDir/Notebooks/Pictures/du.png">
</p>
...
scp File ndombrowski@ada.nioz.nl:/HomeDir

#from server to local
scp ndombrowski@ada.nioz.nl:/HomeDir/File Desktop
...
#compress a file
gzip GCA_002728275.1_ASM272827v1_genomic.fna 
...
## Working on laplace via slurm
######################

As mentioned above on laplace we can not directly run jobs but need to submit them via a job submission system called slurm.
...
#SBATCH --nodelist=no70       # the node we want to work on
...
- time = not necessary for laplace, just make sure you are in the max limit
- mem-per-cpu
...
- ``squeue`` is important to check and see whether your command is running ok but also to see how heavily used laplace is.
...
```bash
grep -c "^Ex" Experiment[012].txt
```

5. [0-9] wildcard = matches matches any number exactly one
```bash
grep -c "^Ex" Experiment[0-9].txt
```
...
```bash
grep -c "[a-z]\{6\}[12]" Experiment[12].txt
```
...
```bash
grep -c "[a-z]\{3,10\}[12]" Experiment[12].txt
```
...
cut -f1 -d "-" File.fna > File_new.fna

#keep the first and second element
cut -f1,2 -d "-" File.fna > File_new.fna
```
...
In this example combine Ex1.txt and Ex2.txt into one file named All_experiments

```bash
cat Experiment1.txt Experiment2.txt > All_experiments.txt
```
...
sort -k4 Experiment2.txt

#sort using the 5th and then the third column
sort -k5 -k3 Experiment1.txt
```
...
```bash
#only keep duplicate
sort Experiment2.txt | uniq -d

#remove duplicate
sort Experiment2.txt| uniq -u
```
...
```bash
cat Experiment[12].txt | sort | uniq
```
...
**Warning**
Based on the file system there are two versions on how to use rename:
...
```bash
perl replace_tree_names.pl names_to_replace.txt File.fasta > File_renamed.fasta
```
...
for sample in `cat List`; do cp ${sample}.fna new_dir/fna; done  

#2. by grepping files using file names
for sample in *`cat List`*fna; do cp $sample new_dir/fna; done  
...
######################
```bash
# search for 'ow' and replace it with 'aagh'
...
sed 's/control[0-9]/control/g' Experiment1.txt > new_file.txt
```
...
```bash
sed 's/N\[0.4uM\]/N(0.4uM)/g' Experiment1.txt > new_file.txt
```
...
#delete the 2nd occurence of each pattern (here *u*)
sed 's/u//2' file
...
```bash
LC_ALL=C join -a1  -j1 -e'-' -o 0,2.2,2.3 <(LC_ALL=C sort File1) <(LC_ALL=C sort File2) -t $'\t' | LC_ALL=C sort  > MergedFile
```
...
Bayesian trees. Notice, needs special setup to run on laplace. Will be added later.
